...
KNOWLEDGE_CATALOG_SCHEMA_VERSION = 3
...
def extract_runtime_guidance_from_payload(payload: Any) -> Optional[dict[str, Any]]:
    if not isinstance(payload, dict):
        return None
...
def resolve_workstation_status_intent(query: Optional[str], explicit_intent: Optional[str] = None) -> str:
    ...
    if any(token in text for token in ("不要动资金", "不要动资产", "don't move funds", "dont move funds", "non-financial", "risk")):
        return "safety"
    if any(token in text for token in ("知识", "过时", "stale", "review queue", "upstream", "source drift")):
        return "review-queue"
...
def predict_runtime_guidance(executed_steps: list[dict[str, Any]]) -> tuple[Optional[dict[str, Any]], list[dict[str, Any]]]:
    ...
    if isinstance(persona_step, dict):
        ...
        if persona_ok or persona_error_code == "PERSONA_COOLDOWN":
            ...
            message = f"Predict persona 已设为 {persona_name or 'custom'}。下一步直接启动 loop，不再手动逐笔提交。"
            if persona_error_code == "PERSONA_COOLDOWN":
                message = "Predict persona 当前处于 cooldown，继续用现有 persona 启动 loop 即可。"
            guidance = {
                "message": message,
    ...
    if isinstance(status_payload, dict) and persona_value in {"", "none", "null"}:
        ...
        guidance = {
            "message": "Predict 已满足基础运行条件，但还没有 persona。先选一个分析风格，再启动 loop。",
            "userActions": [item["label"] for item in follow_up],
            "actionMap": {item["label"]: item["command"] for item in follow_up if item.get("command")},
    ...
    if isinstance(status_payload, dict) and persona_value not in {"", "none", "null"}:
        ...
        guidance = {
            "message": f"Predict persona 已设为 {persona_value}。下一步直接启动 loop，不再手动逐笔提交。",
            "userActions": [item["label"] for item in follow_up],
            "actionMap": {item["label"]: item["command"] for item in follow_up if item.get("command")},
            "nextCommand": ["predict-agent", "loop", "--interval", "120", "--agent-id", "predict-worker"],
            "nextAction": "start_predict_loop",
    ...
    if skill_key == "gov":
        for item in probe_results:
            ...
            if code == "STATE_PRINCIPAL_NOT_IN_EPOCH":
                blockers.append("Principal has no AWP Power this epoch, so Gov signed actions are blocked until stake/power is available")
            elif detail:
...
def predict_persona_hint(state: dict[str, Any], inspection: dict[str, Any]) -> Optional[str]:
    ...
    for item in load_active_processes(state):
        ...
        if label.startswith("启动 Predict loop") or argv[:2] == ["predict-agent", "loop"]:
...
def public_capability_view(report: dict[str, Any]) -> dict[str, Any]:
    return project_fields(report, CAPABILITY_PUBLIC_FIELDS)


def public_playbook_view(playbook: dict[str, Any]) -> dict[str, Any]:
    return project_fields(playbook, PLAYBOOK_PUBLIC_FIELDS)
    ...
    *,
    ...
    if preferred_runnable and preferred_profile and preferred_profile["key"] != "mine":
        ...
        if preferred_profile["key"] == "predict":
            recommendation = f"你已经把 {preferred_name} 设为默认 WorkNet，当前可以先继续它；如果想走更稳的无 stake 路线，Mine 仍然是备选。"
        else:
            recommendation = f"你已经把 {preferred_name} 设为默认 WorkNet，当前建议优先跑它。"
        label = f"只跑 {preferred_name}"
        actions.append(
            {
                "label": label,
                "description": "按你保存的默认 WorkNet 优先启动。",
            }
        )
        ...
        if preferred_profile["key"] == "predict":
            actions.append(
                {
                    "label": "启动 Predict 静默 loop",
                    "description": "让 Predict 在后台持续运行，不逐轮打断你。",
                }
            )
            action_map["启动 Predict 静默 loop"] = run_worknet_command("predict", execute=True, auto_advance=True)
        if mine and mine.get("runnable"):
            actions.append(
                {
                    "label": "改回只跑 Mine",
                    "description": "如果你想回到更稳的无 stake 数据工作流，可以切回 Mine。",
                }
            )
        ...
        recommendation = (
            "建议先跑 Mine，同时把 Predict 挂到静默 loop 里。"
            if predict_ready
            else "建议先跑 Mine，同时让 Predict 观察 24 小时。"
        )
        actions.append(
            {
                "label": "只跑 Mine",
                "description": "先从无 stake 的数据工作流开始。",
            }
        )
        ...
        first = no_stake[0]
        recommendation = f"建议先跑 {first.get('name')}，因为它当前不需要 stake。"
        label = f"只跑 {first.get('name')}"
        actions.append({"label": label, "description": "先从当前最容易启动的工作流开始。"})
    ...
    if predictable and not predictable.get("runnable"):
        actions.append(
            {
                "label": "观察 Predict",
                "description": "先整理预测市场上下文，等官方 runtime 明确后再执行。",
            }
        )
        action_map["观察 Predict"] = "python3 scripts/query-knowledge.py --topic predict"
...
def build_source_drift_report(
    *,
    ...
    for key in all_keys:
        ...
        if previous is None:
            status = "no-baseline"
            note = "No previous snapshot was available for this source key."
        elif current is None:
            status = "missing-current"
            changed_fields.append("results")
            note = "The source key was present in the previous snapshot but not in the current one."
        else:
            ...
            if status == "availability_changed":
                note = "The source changed from reachable to unreachable, or vice versa."
            elif status == "content_changed":
                note = "The fetched content hash or size changed since the last snapshot."
            elif status == "metadata_changed":
                note = "HTTP metadata changed even though the content hash may be stable."
        if status != "unchanged":
            changed += 1
        ...
        impacts.append(
            {
                ...
                "reviewHint": (
                    "Re-read the affected upstream source and re-check the linked topic dossiers, facts, and runtime guidance."
                    if impacted_dossiers or impacted_facts or impacted_evidence or impacted_worknets
                    else "The source changed, but no local dossier/fact/evidence mapping currently points to it."
                ),
                "reviewCommands": {
        ...
        reports.append(
            {
                ...
                "reason": join_sentences(reason_parts),
                "localSkillPath": local_skill_path,
                "installStatus": skill_registry.get(profile["key"], {}).get("status"),
                "apiStatus": "unknown",
                "cliStatus": inspection_status,
                "canStartWithoutStake": True if profile["key"] == "predict" and predict_loop_is_ready else profile["min_stake"] in (None, 0),
...
def build_preflight_report() -> dict[str, Any]:
    ...
    plain_language_summary = (
        "AWP workstation will use the agent work wallet, scan worknets, and ask before any value-moving action."
    )
    if knowledge_review_queue_summary.get("hasPendingReviews"):
        plain_language_summary += (
            f" Recent upstream changes also left {int(knowledge_review_queue_summary.get('pendingReviewCount', 0) or 0)}"
            " knowledge review item(s) pending."
        )
    report = {
        "walletReady": wallet.get("walletReady", False),
        "registered": registered,
            ...
            user_actions.append(
                {
                    ...
                    "description": "直接按你保存的默认 WorkNet 生成 playbook 并进入默认执行路径。",
                }
            )
...
def build_work_playbook(worknet_identifier: str) -> dict[str, Any]:
    ...
    playbook = {
        ...
        "commands": commands,
        "successMetrics": profile.get("success_metrics", []),
        "failureModes": profile.get("failure_modes", []),
        "humanConfirmations": profile.get("human_confirmations", []),
...
def runtime_guidance_from_step(step: dict[str, Any]) -> Optional[dict[str, Any]]:
    return extract_runtime_guidance_from_payload(step_result_payload(step))
...
def reward_hints_from_run(latest_run: dict[str, Any]) -> list[str]:
    ...
    if worknet_key == "mine":
        ...
        hints.append("Gov 暂时没有可靠收益估计；先看公开 markets，等有 AWP Power 后再进入签名动作。")
    elif worknet_key == "ardi":
        hints.append("Ardi 暂时没有可靠收益估计；先补 gas 和 stake，再进入 commit/reveal/inscribe 节奏。")
    return hints
...
def public_earnings_hint_for_worknet(state: dict[str, Any], worknet_key: str) -> Optional[str]:
    ...
    epoch_suffix = ""
    if epochs:
        epoch_suffix = f"（最近可见 epoch: {', '.join(str(item) for item in epochs)}）"
    if len(formatted) == 1:
        return f"{name} 最近一个公开 epoch 的可见 payout 样本约为 {formatted[0]}{epoch_suffix}；这是公开样本，不是你的已实现收益。"
    return (
        f"{name} 最近 {len(formatted)} 个公开 epoch 的可见 payout 样本约在 {formatted[-1]} 到 {formatted[0]} 之间"
...
def build_recovery_decision(
    ...
    *,
    ...
    def prioritize_actions(preferred_label: Optional[str]) -> list[dict[str, Any]]:
        label = str(preferred_label or "").strip()
        if not label:
            return resolved_actions
        first = [item for item in resolved_actions if str(item.get("label")) == label]
        rest = [item for item in resolved_actions if str(item.get("label")) != label]
        return first + rest
    ...
    if (
        ...
        resolved_actions = prioritize_actions(primary_action_label)
        return {
    ...
    if recovery.get("oldRunStopped"):
        ...
        if restart_label:
            ...
            message += f" 如果你现在就要继续，先用“{primary_label}”。"
...
def humanize_review_step(worknet_key: str, step: dict[str, Any], payload: Any) -> Optional[str]:
    ...
    if worknet_key == "mine":
        if label == "mine agent status":
            return "Mine 运行环境已经就绪。"
        if label == "mine control status":
            return "Mine 当前没有活跃 session，等你选定 dataset 后就能开始。"
        if label == "start mine worker" and state == "selection_required":
            return "Mine 已进入 dataset 选择阶段，下一步只需要选一个 dataset 开始第一轮采集。"
        if label == "start mine worker" and message:
            return f"Mine worker 已返回下一步提示：{message}"
    if worknet_key == "gov":
        if label == "gov public markets":
            return "Gov 已读取本期公开 markets 和时间窗口。"
        if label == "gov phase-aware helper":
            phase = payload.get("phase") if isinstance(payload, dict) else None
            if isinstance(phase, str) and phase.strip():
                return f"Gov 已确认当前 phase 是 {phase}，先按这一阶段的公开动作观察。"
...
def humanize_worknet_switch_summary(profile: dict[str, Any], report: Optional[dict[str, Any]]) -> str:
    ...
    if key == "mine":
        if runnable:
            return f"{name} 当前可以直接切过去，而且默认就是无 stake 的数据工作流。"
        return f"{name} 还没准备好自动运行，先检查本地 runtime 和 dataset 入口。"
    if key == "predict":
        if runnable:
            return f"{name} 可以切过去观察市场，也可以直接挂静默 loop；真实收益仍要等市场结算。"
...
def summarize_worknet_review(
    ...
    if worknet_key == "gov":
        ...
        if phase and no_power:
            return {
                "workDone": [
                    f"Gov 已读取本期 markets，并确认当前处于 {phase} phase。"
                ],
                "failures": [
        ...
        if phase:
            return {
                "workDone": [
                    f"Gov 已确认当前处于 {phase} phase，并准备按这一阶段继续观察。"
                ]
            }
    return {}
...
def build_daily_summary(
    ...
    parts: list[str] = []
    if isinstance(headline, str) and headline.strip():
        parts.append(headline.strip().rstrip("."))
    if estimated_rewards:
        parts.append(str(estimated_rewards[0]).strip().rstrip("."))
    if user_actions:
        first_action = str(user_actions[0]).strip()
        if first_action:
            parts.append(f"下一步：{first_action}")
    if not parts:
        return None
    return " ".join(parts) + "。"
...
def strategy_changes_from_guidance(guidance: Any) -> list[str]:
    if not isinstance(guidance, dict):
        return []
    next_action = str(guidance.get("nextAction") or "")
    mapping = {
        "stake_required": "先解决 stake 资格，再继续自动循环。",
        "retry_stake_check": "先重试 stake 资格检查，确认 runtime 返回稳定结果后再继续。",
        "acquire_awp_power_or_observe_gov": "Gov 先做公开观察，等 AWP Power 到位后再碰签名交易和投票。",
        "fund_gas_and_or_satisfy_stake": "Ardi 先补 gas 和 stake，再进入 commit/reveal/inscribe 节奏。",
        "fetch_context": "Predict 先拉市场 context，再决定是否继续下单或观察。",
        "confirm_predict_submission": "Predict 已准备好推荐市场，但正式提交前要先确认方向、tickets 和 reasoning。",
        "wait_for_predict_market": "Predict 当前没有可提交市场，先等待下一轮，再重新拉 context。",
    ...
    *,
    ...
    if action.get("requiresConfirmation"):
        ...
        if action.get("longRunning"):
            ...
            guidance = extract_runtime_guidance_from_payload(step["result"]["stdout"])
            if guidance:
    ...
    def finalize_run_response(response: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            **response,
            ...
            "recoveryDecision": briefing.get("recoveryDecision"),
        }
...
def build_epoch_review() -> dict[str, Any]:
    ...
    for step in latest_run.get("executedSteps", []):
        ...
        if status in {"planned", "ok"} and runtime_payload_has_blocker(payload):
            ...
            detail = humanize_review_step(worknet_key, step, payload) or message or status
            ...
            detail = "started in background"
            if isinstance(headline, str) and headline.strip():
                qualifier = "latest status" if observation.get("alive") else "last observed status"
            ...
            if summary_state == "llm_error":
                failures.append(f"{label}: {summary_detail or headline or 'background loop reported an LLM error'}")
    ...
    for queued in confirmation_items:
        user_actions.append(f"confirm {queued.get('label')}")
    ...
    if active_background_count == 1:
        append_unique_text(user_actions, f"暂停当前运行: {workstation_pause_command(execute=True)}")
    ...
    for observation in background_observations:
        ...
        if label not in processed_background_labels:
            detail = headline.strip() if isinstance(headline, str) and headline.strip() else "background task observed in workstation state"
        ...
        if observation.get("alive"):
            append_unique_text(
                user_actions,
                f"{review_background_action_label(label, inspect=True)}: {workstation_background_command(label, tail_lines=80)}",
            )
            append_unique_text(
                user_actions,
                f"{review_background_action_label(label, stop=True)}: {workstation_background_command(label, stop=True, execute=False)}",
            )
        elif restart_label and restart_label == label:
            append_unique_text(
                user_actions,
                f"{review_background_action_label(label, restart=True)}: {workstation_follow_up_command(label, execute=True)}",
            )
    if not estimated_rewards:
        estimated_rewards.append("reward estimate unavailable until worknet receipts or earnings APIs are present")
    if failures:
        append_unique_text(strategy_changes, "prefer verified local runtimes before enabling unattended execution")
    if has_confirmation_actions:
        append_unique_text(strategy_changes, "leave financial or irreversible actions in the confirmation queue until the user approves them")
    for item in strategy_changes_from_guidance(runtime_guidance):
        if (
    ...
    daily_summary = build_daily_summary(worknet_key, headline, estimated_rewards, user_actions)
    review = {
        "generatedAt": now_iso(),
...
def build_workstation_status(
    *,
    ...
    if resolved_intent == "status":
        ...
        if target_profile is not None:
            ...
            if runnable:
                ...
                append_user_action(
                    ...
                    description="直接按这个 WorkNet 的默认节奏启动；若碰到资金动作仍会先进入确认队列。",
        ...
        headline = "知识待重审队列"
        answer = str(knowledge_review_queue_summary.get("headline") or "当前没有待重审的知识条目。")
        status = "stale_knowledge_pending" if knowledge_review_queue_summary.get("hasPendingReviews") else "knowledge_fresh"
        worknet_key = None
        worknet_name = None
        append_user_action(
            user_actions,
...
def build_coverage_audit() -> dict[str, Any]:
    ...
    for requirement in KNOWLEDGE_COVERAGE_REQUIREMENTS:
        ...
        if key == "trigger-surface":
            ...
            command_labels = [item.get("label") for item in registration_plan.get("commands", [])]
            if registration_plan.get("nextAction") in {"run_awp_skill_registration", "registration_ready"}:
                status = "covered"
            ...
            inspections_payload = build_skill_inspection_catalog(state=state)
            inspections = {
...
def build_knowledge_catalog() -> dict[str, Any]:
    state = state_context()
    preferences = ensure_user_preferences(state)
    inventory = build_source_inventory()
    skill_registry = skill_registry_from_inventory(inventory, state=state)
